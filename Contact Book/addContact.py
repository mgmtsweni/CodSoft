...
def database():
    if nameEntry.get() == '' or numberEntry.get() == '' or \
        ...
        try:
            query = 'SELECT * FROM Contacts WHERE store_name = ? AND id = ?'
...
emailAddress.grid(row=4,column=0)
